...
function tunisyndic_ClientArea(array $params)
{
    ...
    $domain = $domain . ":" . $port;
    $form = sprintf("<form action=\"#\"><h3>change your instance name </h3><h3 id=\"simpleName\">".$params['customfields']['comments']."</h3><h3> To : </h3>
        ...
        if($array->instance){
            ...
            if($res = $mysqli->query("SELECT MAX(id) FROM tblcustomfieldsvalues ")){
                    foreach ($res as $columnInfo) {
                        ...
                        $LastId = $columnInfo['MAX(id)']-2;
                        $IdComment =  $columnInfo['MAX(id)'];
                    }
                }
                if($row == NULL){
                ...
                $mysqli->query("UPDATE `tblcustomfieldsvalues` SET `value`='".$array->instance."' WHERE id = ".$IdComment." ");
                }
        }
    } else {
        $dataarray = explode("\n", $data);
